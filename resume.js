...
time();
alert("You have been drafted to fight in the Forest War. Use arrow keys to move and space to launch a bullet. Good luck...");
...
alert("You are at X: " + (-matrix4.m41 - 1000) + " Z: " + (7500 + matrix4.m43));
	}
	if(e.key == "r"){
		var sol1 = document.getElementsByClassName("soldier");
var matrix3 = new WebKitCSSMatrix(window.getComputedStyle(sol1[ps]).transform);
alert("A soldier is at X: " + matrix3.m41 + " Z: " + (-matrix3.m43));
	...
	if (e.key == "ArrowUp") {
		...
		document.getElementById("universe").style.transform = "translateY("+y+"px) rotateY("+ ry +"deg) translateZ("+ a +"px) perspective("+ (a * -2) +"px) translateX(" + b + "px)";
	}
	if (e.key == "ArrowDown") {
		e.preventDefault();
		a -= 50;
	...
	if(e.key == " "){
		...
		document.getElementById("bullet").style.transform = "translateZ("+ parseInt(far) + "px)";
		...
		alert(matrix.m41 + " " + (matrix2.m41 - (-matrix4.m41 - 1000)) + " " + matrix.m43 + " " + (-matrix2.m43 - (-a + matrix4.m43)))
			if(matrix.m41 === (matrix2.m41 - (-matrix4.m41 - 1000)) && matrix.m43 === (-matrix2.m43 - (-a + matrix4.m43))){
				alert("Hit!");
				pclick(sold[ps]);
...
for(var s = 0; s< (100 - ps); s++){
	...
	soldier.style.transform = "translateY(800px) perspective(6000px) translateX(" + ( 50 * Math.floor(Math.random() * 40)) + "px) translateZ(" + (-50 * Math.floor(Math.random() * 40)) + "px)";
